    ...
    return (
        <div className='h-40 space-y-0.5 md:space-y-2'>
            ...
            <div className='group relative md:ml-2'>
                ...
                h-9 w-9 cursor-pointer opacity-0 transition hover:scale-125 group-hover:opacity-100 ${!isMoved && 'hidden'}`}
                                 onClick={() => handleClick('left')}/>

                <div ref={rowRef}
                     className='flex items-center scrollbar-hide space-x-3 overflow-hidden overflow-x-scroll md:space-x-2.5'>
                    {movies.map(movie => <Thumbnail key={movie.id} movie={movie}/>)}
                </div>

                <ChevronRightIcon className={`absolute top-0 bottom-0 right-2 z-40 m-auto
